...
static byte                        LCD_CACHE[LCD_CACHE_SIZE];
static int                         LWM;
static int                         HWM;
static byte                        LCD_UPD_FLAG;

void wait(void) {
  for (int i = -32000; i < 32000; i++);
}

static void lcd_send_data(byte data) {
...
static void lcd_send_cmd(byte data) {
  LCD_PORT &= ~(_BV(LCD_CE_PIN));
  LCD_PORT &= ~(_BV(LCD_DC_PIN));
  SPDR = data;
  while ((SPSR & 0x80) != 0x80);
...
void lcd_put_char(byte x0, byte y0, unsigned char ch, Color c){
  for(int i = 0;i < 5;i++){
    for(int j = 0;j < 8;j++){
      if((pgm_read_byte(&CHARS_TABLE[(ch - 32) * 5 + i]) >> j) & 1) lcd_pixel(x0 + i, y0 + j, c);
    }
  }
}
